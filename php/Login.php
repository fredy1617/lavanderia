...
class Login
{
    ...
    private function dologinWithPostData()
    {
        ...
        if (empty($_POST['usuario'])) {
            $this->errors[] = "El especio usuario esta vacio.";
        } elseif (empty($_POST['password'])) {
            $this->errors[] = "El especio password esta vacio.";
        } elseif (!empty($_POST['usuario']) && !empty($_POST['password'])) {

            // create a database connection, using the constants from config/db.php (which we loaded in index.php)
            $this->db_connection = new mysqli('localhost', 'root', 'root', 'servespecial');
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                // username field)
                $sql = "SELECT id, usuario, password_hash
                        FROM usuarios
